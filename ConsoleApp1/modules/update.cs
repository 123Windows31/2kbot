    ...
    public static class Update
    {
        public static async void Execute()
        {
            ...
            using (var msc = new MySqlConnection(Global.connectstring))
            {
                ...
                // 更新op列表
                cmd.CommandText = $"SELECT * FROM ops WHERE qid IS NOT NULL AND gid IS NOT NULL;";
                ...
                // 更新黑名单
                cmd.CommandText = $"SELECT * FROM blocklist WHERE qid IS NOT NULL AND gid IS NOT NULL;";
                ...
                await reader.CloseAsync();
                // 更新屏蔽列表
                cmd.CommandText = $"SELECT * FROM ignores WHERE qid IS NOT NULL AND gid IS NOT NULL;";
                reader = (MySqlDataReader)await cmd.ExecuteReaderAsync();
                ...
                await reader.CloseAsync();
                // 更新全局op列表
                cmd.CommandText = $"SELECT * FROM g_ops WHERE qid IS NOT NULL;";
                reader = (MySqlDataReader)await cmd.ExecuteReaderAsync();
                ...
                await reader.CloseAsync();
                // 更新全局黑名单
                cmd.CommandText = $"SELECT * FROM g_blocklist WHERE qid IS NOT NULL;";
                reader = (MySqlDataReader)await cmd.ExecuteReaderAsync();
                ...
                await reader.CloseAsync();
                // 更新全局屏蔽列表
                cmd.CommandText = $"SELECT * FROM g_ignores WHERE qid IS NOT NULL;";
                reader = (MySqlDataReader)await cmd.ExecuteReaderAsync();
